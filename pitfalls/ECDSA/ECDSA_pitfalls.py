...
def ECDSA_leaking_k():
    # Alice生成公私钥对并对消息进行签名
    sk, pk = generate_key()
    print("Alice的私钥为:",hex(sk)[2:].rjust(64, '0'))
...
def ECDSA_reusing_k():
    ...
    e2 = int(e2, 16)
    d = (((e1 - e2) * s2) % N * mod_inverse((s1 - s2) % N, N) - e2) * mod_inverse(r, N) % N
    print("Bob推测的d为:", '0x' + hex(d)[2:].rjust(64, '0'))
    print("验证Bob推测的d是否与Alice的sk相等:", True if d == sk else False)
...
def same_k_of_different_users():
    ...
    e1 = int(e1, 16)
    d_A = (s1 * k - e1) % N * mod_inverse(r1, N) % N
    ...
    e2 = int(e2, 16)
    d_B = (s2 * k - e2) % N * mod_inverse(r2, N) % N
...
def verify_Malleability():
    ...
    r, s = signature
    signature_test = (r, -s)
    print("验证(r,-s)是否是合法签名:", True if ECDSA_verify(signature_test, message, pk) == 1 else False)

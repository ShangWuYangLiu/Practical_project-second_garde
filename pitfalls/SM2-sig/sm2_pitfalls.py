...
def sm2_sign_and_assign_k(k, sk, msg, ZA):
    ...
    e = int(e, 16)  # str -> int
    a_dot = elliptic_mult(k, G)  # (x1, y1) = kG
    r = (e + a_dot[0]) % N  # r = (e + x1) % n
    s = 0
    if r != 0 and r + k != N:
        s = (mod_inverse(1 + sk, N) * (k - r * sk)) % N
    if s != 0:  return (r, s)
...
def sm2_leaking_k():
    ...
    # 伪造签名
    ID_for_forge = ID
    pk_from_d = elliptic_mult(d, G)
    ZA_for_forge = pre_compute(ID_for_forge, A, B, G_X, G_Y, pk_from_d[0], pk_from_d[1])
    forged_signature = sign(d, message_for_forge, str(ZA_for_forge))
    # 用Alice的公钥验证Bob伪造的签名
    print("验证Bob是否通过d伪造签名成功:",
          True if verify(pk, ID_for_forge, message_for_forge, forged_signature) == 1 else False)
    if verify(pk, ID_for_forge, message_for_forge, forged_signature) == 1:
...
def same_k_of_different_users():
    ...
    ZA = pre_compute(ID_A, A, B, G_X, G_Y, pk_A[0], pk_A[1])
